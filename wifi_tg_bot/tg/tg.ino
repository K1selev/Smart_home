...
void loop() {
  int lightSensorValue = analogRead(A0);  // Значение освещенности (0-1023)
  bool allowLight = lightSensorValue < 700;  // Включать, если темно
    ...
    if (command.startsWith("rgb:")) { // Пример: rgb:1,255,100,50,0.8
      ...
      esp.println(out);  // отправляем одной строкой
    }
